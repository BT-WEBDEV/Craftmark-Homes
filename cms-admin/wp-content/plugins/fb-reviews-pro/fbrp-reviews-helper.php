...
function fbrp_page_reviews($page_id, $reviews, $hide_avatar, $text_size, $min_filter, $min_letter, $pagination, $disable_user_link, $open_link, $nofollow_link, $lazy_load_img) {
    ...
    if (count($reviews) > 0) {
        ...
        foreach ($reviews as $review) {
            $review_rating = fbrp_get_review_rating($review);
            if ($review_rating >= $min_filter && (!$min_letter || (isset($review->review_text) && strlen($review->review_text) > $min_letter))) {
...
function fbrp_page_review($page_id, $review, $hide_avatar, $text_size, $disable_user_link, $open_link, $nofollow_link, $lazy_load_img, $hide_review=false) {
    ...
    <div class="wp-facebook-review <?php if ($hide_review) { ?>wp-facebook-hide<?php } ?>">
        <?php if (!$hide_avatar) { ?>
        <div class="wp-facebook-left">
            <?php fbrp_image($reviewer_photo, $reviewer_name, $lazy_load_img); ?>
        </div>
        <?php } ?>
        <div class="wp-facebook-right">
            <?php
            if (!$disable_user_link) {
                ...
                fbrp_anchor($profile_url, 'wp-facebook-name', $reviewer_name, $open_link, $nofollow_link);
            } else {
            ...
            <div class="wp-facebook-feedback">
                <span class="wp-facebook-stars"><?php echo fbrp_stars($review_rating); ?></span>
...
function fbrp_get_review_rating($review) {
